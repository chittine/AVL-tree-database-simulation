...
class AVLTree:
    ...
    def right_rotate(self, root: Node) -> Optional[Node]:
        ...
        # right child has been pulled up to new root -> push old root down right, update parent
        new_root.right = root
        new_root.parent = root.parent
        if root.parent is not None:
            if root is root.parent.left:
...
class Table:
    ...
    def insert_rows(self, values: List[List[str]], attributes: List[str]) -> None:
        ...
        for row in values:
            ordered_row = []
            for attribute in self.header_row:
                ordered_row.append(row[self.header_row.index(attribute)-1])
            self.data_tree.insert(self.data_tree.origin,self.insertion_index,ordered_row)
    ...
    def remove_rows(self, indices: List[str]) -> None:
        ...
        for index in indices:
            self.data_tree.remove(int(index))

    def show_latest(self) -> List[List[str]]:
        """
        FILL OUT DOCSTRING
        """
        if self.data_tree.size == 0:
            return [self.header_row]
        latest_node = self.data_tree.max(self.data_tree.origin)
        latest_row = [str(latest_node.value)] + latest_node.data
    ...
    def show_oldest(self) -> List[List[str]]:
        ...
        oldest_row = [str(oldest_node.value)] + oldest_node.data
    ...
    def show_everything(self) -> List[List[str]]:
        ...
        for node in self.data_tree:
            row = [str(node.value)] + node.data
            rows.append(row)
        return rows
